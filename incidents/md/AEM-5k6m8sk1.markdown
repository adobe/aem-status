...
Because of our long CDN cache TTLs, a page only degraded once it dropped out of cache and had to be re-rendered. High-traffic pages stay cached the longest, so the defect surfaced gradually and unevenly rather than site-wide at the moment of deployment. We learned of it when the first customer reported missing metadata at 09:13 UTC on September 21, just under 2 days and 6 hours after the regression reached production: no monitoring alert fired, and none could have, since no requests failed and the only symptom was correct-looking pages with incomplete metadata.

### Root Cause

A change intended to let internal callers load a single metadata partition moved an assignment so that the site's configured list of metadata sheets was overwritten before those sheets were fetched. The loader read that list back out of the object it had just been assigned into, so by the time it ran, the configured sources were gone, and it fell back to its built-in default of a single sheet. The library containing this change was published on the evening of September 18, but the trigger for the incident was the automated dependency update that deployed it into the delivery path early on September 19 — until then, nothing in production had changed.
